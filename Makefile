...
#   GDK_NVML_LIB_PATH= path to CUDA GDK (stub) library path, so $(GDK_NVML_LIB_PATH)/libnvidia-ml.$(LIBRARY_EXTENSION) exists
...
LIBPATH:= -fopenmp -I/usr/local/openmpi/include
LIBS_LIST:=
LDFLAGS:= -L/usr/local/openmpi/lib -lmpi
...
PERF_PROFILER_LIB:= $(LIBDIR)/lib$(PERF_PROFILER).$(LIBRARY_EXTENSION)
...
CNTKMATH_LIB:= $(LIBDIR)/lib$(CNTKMATH).$(LIBRARY_EXTENSION)
...
CNTKLIBRARY_LIB:=$(LIBDIR)/lib$(CNTKLIBRARY).$(LIBRARY_EXTENSION)
...
CPP_EXTENSIBILITY_EXAMPLES_LIB:= $(LIBDIR)/Cntk.ExtensibilityExamples-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
BINARY_CONVOLUTION_EXAMPLE_LIB:= $(LIBDIR)/Cntk.BinaryConvolutionExample-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
PROPOSAL_LAYER_LIB:= $(LIBDIR)/Cntk.ProposalLayerLib-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
EVAL_LIB:=$(LIBDIR)/lib$(EVAL).$(LIBRARY_EXTENSION)
ALL_LIBS+=$(EVAL_LIB)
SRC+=$(EVAL_SRC)
...
HTKMLFREADER:=$(LIBDIR)/Cntk.Reader.HTKMLF-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
COMPOSITEDATAREADER:=$(LIBDIR)/Cntk.Composite-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
HTKDESERIALIZERS:=$(LIBDIR)/Cntk.Deserializers.HTK-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
LMSEQUENCEREADER:= $(LIBDIR)/Cntk.Reader.LMSequence-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
LUSEQUENCEREADER:=$(LIBDIR)/Cntk.Reader.LUSequence-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
UCIFASTREADER:=$(LIBDIR)/Cntk.Reader.UCIFast-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
LIBSVMBINARYREADER:=$(LIBDIR)/Cntk.Reader.SVMBinary-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
SPARSEPCREADER:=$(LIBDIR)/Cntk.Reader.SparsePC-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
CNTKBINARYREADER:=$(LIBDIR)/Cntk.Deserializers.Binary-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
CNTKTEXTFORMATREADER:=$(LIBDIR)/Cntk.Deserializers.TextFormat-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
KALDI2READER:=$(LIBDIR)/Cntk.Reader.Kaldi2-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
IMAGEREADER:=$(LIBDIR)/Cntk.Deserializers.Image-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
IMAGEWRITER:=$(LIBDIR)/Cntk.ImageWriter-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
...
MULTIVERSO_LIB:=$(LIBDIR)/libmultiverso.$(LIBRARY_EXTENSION)
...
PYTHON_EXTRA_LIBS_BASENAMES:=$(addsuffix .$(LIBRARY_EXTENSION),$(addprefix lib,$(filter-out $(PYTHON_LIBS_EXCLUDE_LIST),$(PYTHON_LIBS_LIST))))
...
JAVA_LIBRARY_OUTPUT=linux

ifeq ("$(ARCH)","Darwin")
JAVA_LIBRARY_OUTPUT=macos
endif

GENERATED_JAVA_DIR=$(JAVA_SWIG_DIR)/com/microsoft/CNTK
JDK_BIN_PATH=$(JDK_PATH)/bin
JDK_INCLUDE_PATH:=$(JDK_PATH)/include
JDK_INCLUDE_PATH+=$(JDK_INCLUDE_PATH)/$(JAVA_LIBRARY_OUTPUT)

JAVA_SO_NAME=$(LIBDIR)/libCntk.Core.JavaBinding-$(CNTK_COMPONENT_VERSION).$(LIBRARY_EXTENSION)
JAVA_LOAD_DEPS:=$(CNTKMATH_LIB) $(PERF_PROFILER_LIB) $(CNTKLIBRARY_LIB) $(JAVA_SO_NAME)
JAVA_LOAD_DEPS:=$(JAVA_LOAD_DEPS:$(LIBDIR)/%=%)
JAVA_DEP_SO_NAMES_GPU:=libcublas.$(LIBRARY_EXTENSION) libcudart.$(LIBRARY_EXTENSION) libcurand.$(LIBRARY_EXTENSION) libcusparse.$(LIBRARY_EXTENSION)
...
java: $(JAVA_LIBS)
	...
	mkdir -p $(JAVA_SWIG_DIR)/com/microsoft/CNTK/lib/$(JAVA_LIBRARY_OUTPUT)
	echo $(JAVA_SO_NAME:$(LIBDIR)/%=%) > $(JAVA_SWIG_DIR)/com/microsoft/CNTK/lib/$(JAVA_LIBRARY_OUTPUT)/NATIVE_LOAD_MANIFEST
	for so in libiomp5.$(LIBRARY_EXTENSION) libmklml_intel.$(LIBRARY_EXTENSION); do \
	    cp -p $(MKL_LIB_PATH)/$$so $(JAVA_SWIG_DIR)/com/microsoft/CNTK/lib/$(JAVA_LIBRARY_OUTPUT); \
	    echo $$so >> $(JAVA_SWIG_DIR)/com/microsoft/CNTK/lib/$(JAVA_LIBRARY_OUTPUT)/NATIVE_MANIFEST; \
	done
	for so in $(JAVA_LOAD_DEPS); do \
	    cp -p $(LIBDIR)/$$so $(JAVA_SWIG_DIR)/com/microsoft/CNTK/lib/$(JAVA_LIBRARY_OUTPUT); \
	    echo $$so >> $(JAVA_SWIG_DIR)/com/microsoft/CNTK/lib/$(JAVA_LIBRARY_OUTPUT)/NATIVE_MANIFEST; \
	done
ifdef CUDA_PATH
	for so in $(JAVA_DEP_SO_NAMES_GPU); do \
	    cp -p $(CUDA_PATH)/lib64/$$so $(JAVA_SWIG_DIR)/com/microsoft/CNTK/lib/$(JAVA_LIBRARY_OUTPUT); \
	    echo $$so >> $(JAVA_SWIG_DIR)/com/microsoft/CNTK/lib/$(JAVA_LIBRARY_OUTPUT)/NATIVE_MANIFEST; \
	done
	cp -p $(CUDNN_PATH)/cuda/lib64/libcudnn.$(LIBRARY_EXTENSION) $(JAVA_SWIG_DIR)/com/microsoft/CNTK/lib/$(JAVA_LIBRARY_OUTPUT)
	echo 'libcudnn.$(LIBRARY_EXTENSION)' >> $(JAVA_SWIG_DIR)/com/microsoft/CNTK/lib/$(JAVA_LIBRARY_OUTPUT)/NATIVE_MANIFEST
	cp -p $(GDK_NVML_LIB_PATH)/libnvidia-ml.$(LIBRARY_EXTENSION) $(JAVA_SWIG_DIR)/com/microsoft/CNTK/lib/$(JAVA_LIBRARY_OUTPUT)
	echo 'libnvidia-ml.$(LIBRARY_EXTENSION)' >> $(JAVA_SWIG_DIR)/com/microsoft/CNTK/lib/$(JAVA_LIBRARY_OUTPUT)/NATIVE_MANIFEST
endif
